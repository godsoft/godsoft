		...
		<script type="text/javascript" language="javascript" defer="defer">
	        ...
	        function init() {
	            if (!navigator.geolocation) {
	                ...
	                $.getJSON("<c:url value='/mbl/com/geo/selectMobileBuildingLocationInfoList.mdo'/>", $('#searchVO').serialize(), function(json){
	                	var html = "";
	                		
	                	html += '<div style="padding:10px 0">';
	                	html += '<div class="egov-grid">';
	                	html += '	<div class="egov-wid2">';
	                	html += '		<div align="left">';							
	                	html += '			<b>MAP</b>';
	                	html += '		</div>';
	                	html += '	</div>';
	                	html += '	<div class="egov-wid10">';
	                	html += '		<div align="right">';	
	                	html += '			<form id="searchVO" name="searchVO" method="post">';
	                	html += '				<p>조회범위';
	                	html += '				<select id="searchCondition" name="searchCondition" onchange="javascript:init();">';
	                	html += '					<option value="100">100m 이내</option>';
	                	html += '					<option value="200">200m 이내</option>';
	                	html += '					<option value="300">300m 이내</option>';
	                	html += '					<option value="500">500m 이내</option>';
	                	html += '					<option value="1000">1km 이내</option>';
	                	html += '					<option value="2000">2km 이내</option>';
	               		html += '				</select></p>';
	               		html += '			</form>';
	               		html += '		</div>';
	               		html += '	</div>';
