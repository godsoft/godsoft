...
public class EgovMblQustnrRespondInfoController {
	...
	public String EgovQustnrRespondInfoManageListView(
			@ModelAttribute("searchVO") ComDefaultVO searchVO,  
			HttpServletRequest request, 
    ...
    throws Exception {
    	...
    	model.addAttribute("occpTyCode", listComCode);
    	 ...
    	 if(loginVO.getUniqId() != null){
 		 ...
 		 //설문템플릿정보 
		 model.addAttribute("qustnrTmplatManage",  (Map)egovQustnrRespondInfoService.selectQustnrTmplatManage(commandMap).get(0));
		 
    	 //설문정보 
    	 model.addAttribute("comtnqestnrinfo",  (Map)egovQustnrRespondInfoService.selectQustnrRespondInfoManageComtnqestnrinfo(commandMap).get(0));
    	 //문항정보
	...
	public String EgovQustnrRespondInfoManageRegistActor(
			@ModelAttribute("searchVO") ComDefaultVO searchVO,
			Map commandMap,  
    		ModelMap model)
    throws Exception {
