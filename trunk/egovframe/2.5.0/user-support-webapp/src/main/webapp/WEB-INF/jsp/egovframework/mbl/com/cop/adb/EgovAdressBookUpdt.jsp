...
<html>
	...
	<head>
		...
		function fn_egov_update_adbkInf(){		
			...
			document.adbk.action = "${pageContext.request.contextPath}/cop/adb/updateAdressBook.mdo";
			...
			<form commandName="adbk" name="adbk" method="post">
				...
				<div data-inline="true">
					<dl class="uss-registOk">
						...
						<dd>
			        		...
			        		<input name="adbkNm" type="text" value='<c:out value="${searchVO.adbkNm}" />' >
			        		</c:if>
						...
						<dd>
							<fieldset data-role="controlgroup" data-type="horizontal"  data-inline="true"> 				
								<input type="radio" name="othbcScope" id="othbcScope1" value="개인" <c:if test="${searchVO.othbcScope == '개인'}"> checked="checked"</c:if> <c:if test="${writer != true}"> disabled </c:if> />
								<label for="othbcScope1">개인</label>
								<input type="radio" name="othbcScope" id="othbcScope2" value="부서" <c:if test="${searchVO.othbcScope == '부서'}"> checked="checked"</c:if> <c:if test="${writer != true}"> disabled </c:if> />
								<label for="othbcScope2">부서</label> 
								<input type="radio" name="othbcScope" id="othbcScope3" value="회사" <c:if test="${searchVO.othbcScope == '회사'}"> checked="checked"</c:if> <c:if test="${writer != true}"> disabled </c:if> />
				...
				<c:if test="${writer == true}">
					<div class="ui-grid-a addBtn">
						<div class="ui-block-a"><a href="javascript:fn_egov_update_adbkInf();" data-role="button" data-theme="b">수정</a></div>
				...
				<c:if test="${writer != true}">
					<div class="com-addBgBtn">
						<a href="javascript:fn_egov_select_adbkInfs();" data-role="button" data-theme="b">목록</a>					
					</div>
				</c:if>
			</form>
