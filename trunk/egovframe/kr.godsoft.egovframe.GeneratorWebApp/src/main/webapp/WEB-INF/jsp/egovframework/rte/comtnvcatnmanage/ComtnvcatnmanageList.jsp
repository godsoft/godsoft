...
<%
 /**
  * @Class Name : ComtnvcatnmanageList.jsp
  * @Description : Comtnvcatnmanage List 화면
...
<!--
/* 글 수정 화면 function */
function fn_egov_select(applcntId, vcatnSe, bgnde, endde) {
	document.getElementById("listForm").applcntId.value = applcntId;
	document.getElementById("listForm").vcatnSe.value = vcatnSe;
	document.getElementById("listForm").bgnde.value = bgnde;
	document.getElementById("listForm").endde.value = endde;
   	document.getElementById("listForm").action = "<c:url value='/comtnvcatnmanage/updateComtnvcatnmanageView.do'/>";
   	document.getElementById("listForm").submit();		
}

/* 글 등록 화면 function */
function fn_egov_addView() {
   	document.getElementById("listForm").action = "<c:url value='/comtnvcatnmanage/addComtnvcatnmanageView.do'/>";
   	document.getElementById("listForm").submit();		
}
...
function fn_egov_link_page(pageNo){
	...
	document.getElementById("listForm").action = "<c:url value='/comtnvcatnmanage/ComtnvcatnmanageList.do'/>";
   	document.getElementById("listForm").submit();
}
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
												...
												<td align="center" class="listtd"><a href="javascript:fn_egov_select('<c:out value="${result.applcntId}"/>, <c:out value="{D}{result.vcatnSe}"/>, <c:out value="{D}{result.bgnde}"/>, <c:out value="{D}{result.endde}"/>')"><c:out value="${result.applcntId}"/></a>&nbsp;</td>
																...
																<td align="center" class="listtd"><c:out value="${result.noonSe}"/>&nbsp;</td>
											</tr>
			</c:forEach>
		</table>
	</div>
