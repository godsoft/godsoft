...
<%
 /**
  * @Class Name : ComtsusersummaryList.jsp
  * @Description : Comtsusersummary List 화면
...
<title>comtsusersummary 목록</title>
...
function fn_egov_select(occrrncDe, statsSe, detailStatsSe) {
	...
	document.getElementById("listForm").detailStatsSe.value = detailStatsSe;
   	document.getElementById("listForm").action = "<c:url value='/comtsusersummary/updateComtsusersummaryView.do'/>";
   	document.getElementById("listForm").submit();		
}

/* 글 등록 화면 function */
function fn_egov_addView() {
   	document.getElementById("listForm").action = "<c:url value='/comtsusersummary/addComtsusersummaryView.do'/>";
   	document.getElementById("listForm").submit();		
}
...
function fn_egov_link_page(pageNo){
	...
	document.getElementById("listForm").action = "<c:url value='/comtsusersummary/ComtsusersummaryList.do'/>";
   	document.getElementById("listForm").submit();
}
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
												...
												<td align="center" class="listtd"><a href="javascript:fn_egov_select('<c:out value="${result.occrrncDe}"/>', '<c:out value="${result.statsSe}"/>', '<c:out value="${result.detailStatsSe}"/>')"><c:out value="${result.occrrncDe}"/></a>&nbsp;</td>
																...
																<td align="center" class="listtd"><c:out value="${result.userCo}"/>&nbsp;</td>
											</tr>
			</c:forEach>
		</table>
	</div>
