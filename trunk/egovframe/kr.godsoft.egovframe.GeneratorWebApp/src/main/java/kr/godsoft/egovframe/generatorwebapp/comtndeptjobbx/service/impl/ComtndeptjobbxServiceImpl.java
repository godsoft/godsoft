package kr.godsoft.egovframe.generatorwebapp.comtndeptjobbx.service.impl;

import java.util.List;

import javax.annotation.Resource;

import org.springframework.stereotype.Service;

import egovframework.rte.fdl.cmmn.AbstractServiceImpl;
import egovframework.rte.fdl.idgnr.EgovIdGnrService;
import kr.godsoft.egovframe.generatorwebapp.comtndeptjobbx.service.ComtndeptjobbxService;
import kr.godsoft.egovframe.generatorwebapp.comtndeptjobbx.service.ComtndeptjobbxDefaultVO;
import kr.godsoft.egovframe.generatorwebapp.comtndeptjobbx.service.ComtndeptjobbxVO;
import kr.godsoft.egovframe.generatorwebapp.comtndeptjobbx.service.impl.ComtndeptjobbxDAO;

/**
 * @Class Name : ComtndeptjobbxServiceImpl.java
 * @Description : Comtndeptjobbx Business Implement class
 * @Modification Information
 *
 * @author 이백행
 * @since 2012.03.13
 * @version 1.0
 * @see
 *  
 *  Copyright (C)  All right reserved.
 */

@Service("comtndeptjobbxService")
public class ComtndeptjobbxServiceImpl extends AbstractServiceImpl implements
        ComtndeptjobbxService {

    @Resource(name="comtndeptjobbxDAO")
    private ComtndeptjobbxDAO comtndeptjobbxDAO;
    
    /** ID Generation */
    //@Resource(name="{egovComtndeptjobbxIdGnrService}")    
    //private EgovIdGnrService egovIdGnrService;

	/**
	 * comtndeptjobbx을 등록한다.
	 * @param vo - 등록할 정보가 담긴 ComtndeptjobbxVO
	 * @return 등록 결과
	 * @exception Exception
	 */
    public String insertComtndeptjobbx(ComtndeptjobbxVO vo) throws Exception {
    	log.debug(vo.toString());
    	
    	/** ID Generation Service */
    	//TODO 해당 테이블 속성에 따라 ID 제너레이션 서비스 사용
    	//String id = egovIdGnrService.getNextStringId();
    	//vo.setId(id);
    	log.debug(vo.toString());
    	
    	comtndeptjobbxDAO.insertComtndeptjobbx(vo);
    	//TODO 해당 테이블 정보에 맞게 수정    	
        return null;
    }

    /**
	 * comtndeptjobbx을 수정한다.
	 * @param vo - 수정할 정보가 담긴 ComtndeptjobbxVO
	 * @return void형
	 * @exception Exception
	 */
    public void updateComtndeptjobbx(ComtndeptjobbxVO vo) throws Exception {
        comtndeptjobbxDAO.updateComtndeptjobbx(vo);
    }

    /**
	 * comtndeptjobbx을 삭제한다.
	 * @param vo - 삭제할 정보가 담긴 ComtndeptjobbxVO
	 * @return void형 
	 * @exception Exception
	 */
    public void deleteComtndeptjobbx(ComtndeptjobbxVO vo) throws Exception {
        comtndeptjobbxDAO.deleteComtndeptjobbx(vo);
    }

    /**
	 * comtndeptjobbx을 조회한다.
	 * @param vo - 조회할 정보가 담긴 ComtndeptjobbxVO
	 * @return 조회한 comtndeptjobbx
	 * @exception Exception
	 */
    public ComtndeptjobbxVO selectComtndeptjobbx(ComtndeptjobbxVO vo) throws Exception {
        ComtndeptjobbxVO resultVO = comtndeptjobbxDAO.selectComtndeptjobbx(vo);
        if (resultVO == null)
            throw processException("info.nodata.msg");
        return resultVO;
    }

    /**
	 * comtndeptjobbx 목록을 조회한다.
	 * @param searchVO - 조회할 정보가 담긴 VO
	 * @return comtndeptjobbx 목록
	 * @exception Exception
	 */
    public List selectComtndeptjobbxList(ComtndeptjobbxDefaultVO searchVO) throws Exception {
        return comtndeptjobbxDAO.selectComtndeptjobbxList(searchVO);
    }

    /**
	 * comtndeptjobbx 총 갯수를 조회한다.
	 * @param searchVO - 조회할 정보가 담긴 VO
	 * @return comtndeptjobbx 총 갯수
	 * @exception
	 */
    public int selectComtndeptjobbxListTotCnt(ComtndeptjobbxDefaultVO searchVO) {
		return comtndeptjobbxDAO.selectComtndeptjobbxListTotCnt(searchVO);
	}
    
}
