...
function fn_egov_draw_barchartgraph(){
	...
	for (var i = 0; i < bar_data.length; i++) {
		for (var j = 0; j < bar_data[i].yaxis.length; j++) {
			...
			left_margin = Math.max(left_margin, (y_gradations[index].toString()).length);
		}
	}
	left_margin = left_margin * (4 * ratio);
	...
	for (var i = 0; i < bar_data.length; i++) {
		...
		for (var j = 0; j < bar_data[i].yaxis.length; j++) {
			...
			x_point = scale_rect[0] + left_margin;
...
function fn_egov_draw_scalemark(context, scale_rect, x_gradations, y_gradations, x_line, y_line){
	...
	context.beginPath();
	context.moveTo(scale_rect[0] + left_margin, scale_rect[1]+scale_rect[3]);
	context.lineTo(scale_rect[0] + left_margin, scale_rect[1]);
	context.moveTo(scale_rect[0] + left_margin, scale_rect[1]+scale_rect[3]);
	context.lineTo(scale_rect[0] + left_margin+scale_rect[2], scale_rect[1]+scale_rect[3]);			
	context.stroke();
	context.closePath();
	...
	if(x_line == true){				
		for(var i = 0; i < x_line_cnt; i++){
			...
			context.fillText(x_gradations[i], scale_rect[0]+(x_line_interval*i)-(x_line_interval*x_label_position) + left_margin, scale_rect[1] + scale_rect[3]+x_line_margin);
		}
		context.fillText(x_gradations[i], scale_rect[0]+(x_line_interval*i)-(x_line_interval*x_label_position) + left_margin, scale_rect[1] + scale_rect[3]+x_line_margin);
	}else{
		for(var i = 0; i < x_line_cnt; i++){
	...
	if(y_line == true){
		...
		for(var i = 0; i < y_line_cnt; i++){
			context.moveTo(scale_rect[0] + left_margin, scale_rect[1]+scale_rect[3]-(y_line_interval*(i+1)));
			context.lineTo(scale_rect[0] + left_margin + scale_rect[2], scale_rect[1]+scale_rect[3]-(y_line_interval*(i+1)));
