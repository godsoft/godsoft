...
<%
 /**
  * @Class Name : ComtneventatdrnList.jsp
  * @Description : Comtneventatdrn List 화면
...
<title>comtneventatdrn 목록</title>
<link type="text/css" rel="stylesheet" href="<c:url value='/css/egovframework/sample.css'/>"/>
<script type="text/javaScript" language="javascript" defer="defer">
<!--
/* 글 수정 화면 function */
function fn_egov_select(applcntId, eventId) {
	document.getElementById("listForm").applcntId.value = applcntId;
	document.getElementById("listForm").eventId.value = eventId;
   	document.getElementById("listForm").action = "<c:url value='/comtneventatdrn/updateComtneventatdrnView.do'/>";
   	document.getElementById("listForm").submit();		
}

/* 글 등록 화면 function */
function fn_egov_addView() {
   	document.getElementById("listForm").action = "<c:url value='/comtneventatdrn/addComtneventatdrnView.do'/>";
   	document.getElementById("listForm").submit();		
}
...
function fn_egov_link_page(pageNo){
	...
	document.getElementById("listForm").action = "<c:url value='/comtneventatdrn/ComtneventatdrnList.do'/>";
   	document.getElementById("listForm").submit();
}
	...
	<div id="table">
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
												...
												<td align="center" class="listtd"><a href="javascript:fn_egov_select('<c:out value="${result.applcntId}"/>', '<c:out value="${result.eventId}"/>')"><c:out value="${result.applcntId}"/></a>&nbsp;</td>
																<td align="center" class="listtd"><a href="javascript:fn_egov_select('<c:out value="${result.applcntId}"/>', '<c:out value="${result.eventId}"/>')"><c:out value="${result.eventId}"/></a>&nbsp;</td>
