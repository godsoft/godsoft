...
<%
 /**
  * @Class Name : ComthhttpmonloginfoList.jsp
  * @Description : Comthhttpmonloginfo List 화면
...
<title>comthhttpmonloginfo 목록</title>
<link type="text/css" rel="stylesheet" href="<c:url value='/css/egovframework/sample.css'/>"/>
<script type="text/javaScript" language="javascript" defer="defer">
<!--
/* 글 수정 화면 function */
function fn_egov_select(sysId, logId) {
	document.getElementById("listForm").sysId.value = sysId;
	document.getElementById("listForm").logId.value = logId;
   	document.getElementById("listForm").action = "<c:url value='/comthhttpmonloginfo/updateComthhttpmonloginfoView.do'/>";
   	document.getElementById("listForm").submit();		
}

/* 글 등록 화면 function */
function fn_egov_addView() {
   	document.getElementById("listForm").action = "<c:url value='/comthhttpmonloginfo/addComthhttpmonloginfoView.do'/>";
   	document.getElementById("listForm").submit();		
}
...
function fn_egov_link_page(pageNo){
	...
	document.getElementById("listForm").action = "<c:url value='/comthhttpmonloginfo/ComthhttpmonloginfoList.do'/>";
   	document.getElementById("listForm").submit();
}
	...
	<div id="table">
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
												...
												<td align="center" class="listtd"><a href="javascript:fn_egov_select('<c:out value="${result.sysId}"/>', '<c:out value="${result.logId}"/>')"><c:out value="${result.sysId}"/></a>&nbsp;</td>
																...
																<td align="center" class="listtd"><a href="javascript:fn_egov_select('<c:out value="${result.sysId}"/>', '<c:out value="${result.logId}"/>')"><c:out value="${result.logId}"/></a>&nbsp;</td>
											</tr>
			</c:forEach>
		</table>
	</div>
