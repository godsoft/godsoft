...
for ($i = 1; $i < 11; $i++) {
    if ($board['bo_' . $i . '_subj'] != '') {
        ...
        if (is_null($write['wr_' . $i]) == true) {
            //$write['wr_' . $i] = sprintf('%010d', ${'wr_' . $i});
            $write['wr_' . $i] = ${'wr_' . $i};
        }
?>

<p style="display: none;"><label for="wr_<?php echo $i; ?>"><?php echo $board['bo_' . $i . '_subj']; ?></label>: <input type="text" name="wr_<?php echo $i; ?>" id="wr_<?php echo $i; ?>" value="<?php echo $write['wr_' . $i]; ?>" style="width: 80%;" /></p>
